---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for nivbot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, nivbot cron scheduling for periodic checks, or version status checks on a machine running nivbot (laptop, workstation, Pi, VPS).
---

# nivbot Host Hardening

## Overview

Assess and harden the host running nivbot, then align it to a user-defined risk tolerance without breaking access. Use nivbot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim nivbot changes the host firewall, SSH, or OS updates; it does not.
...
5. nivbot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if nivbot can access sensitive data.
...
### 2) Run nivbot security audits (read-only)

As part of the default read-only checks, run `nivbot security audit --deep`. Only offer alternatives if the user requests them:

1. `nivbot security audit` (faster, non-probing)
2. `nivbot security audit --json` (structured output)

Offer to apply nivbot safe defaults (numbered):

1. `nivbot security audit --fix`

Be explicit that `--fix` only tightens nivbot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check nivbot version/update status (read-only)

As part of the default read-only checks, run `nivbot update status`.
...
- Credential hygiene notes (location of nivbot creds, prefer disk encryption)
...
After nivbot install or first hardening pass, run at least one baseline audit and version check:

- `nivbot security audit`
- `nivbot security audit --deep`
- `nivbot update status`

Ongoing monitoring is recommended. Use the nivbot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `nivbot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `nivbot update status`
...
Before creating, `nivbot cron list` and match on exact `name`. If found, `nivbot cron edit <id> ...`.
If not found, `nivbot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `nivbot update status` (preferred for source checkouts and channels)
2. `npm view nivbot version` (published npm version)

## nivbot command accuracy

Use only supported commands and flags:

- `nivbot security audit [--deep] [--fix] [--json]`
- `nivbot status` / `nivbot status --deep`
- `nivbot health --json`
- `nivbot update status`
- `nivbot cron add|list|runs|run`

Do not invent CLI flags or imply nivbot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by nivbot compaction:
